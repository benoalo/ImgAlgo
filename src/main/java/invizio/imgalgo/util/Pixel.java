...
public class Pixel {
	...
	public static void getPosFromIdx(long idx, long[] position, long[] dimensions)
	{
	...
	public static long[] getPosFromIdx(long idx, long[] dimensions)
	{
		long[] position = new long[dimensions.length];
		for ( int i = 0; i < dimensions.length; i++ )
		{
			position[ i ] = idx % dimensions[ i ] ;
			idx /= dimensions[ i ];
		}
		return position;
	}
	...
	public static long getIdxFromPos(long[] position, long[] dimensions)
	{
		long idx = 0;
		long step = 1;
