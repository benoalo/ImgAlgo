...
@Deprecated
public class RleCCL_multithreaded2  < T extends RealType<T> & NativeType<T> > extends DefaultLabelAlgorithm<T>
{
	...
	public RleCCL_multithreaded2(RandomAccessibleInterval<T> input, float threshold )
	{
	...
	public RleCCL_multithreaded2(RandomAccessibleInterval<T> input, float threshold, int numberOfThreads )
	{
	...
	protected void process() 
	{
		...
		RleImg runImgFull = new RleImgDefault( input );
		
		for( Chunck<T> chunck : tree )
		{
			if ( chunck.children.size() == 0  )
			{
				RleLabeler worker = new RleLabeler( chunck , runImgFull, input );
				chunck.worker2 = worker;
		...
		for(int i=0; i<tree.size(); i++)
		{
			...
			if( chunck.parent != null) 
			{
				...
				{
					RleMerger merger = new RleMerger(chunck.parent , runImgFull );
					chunck.parent.worker2 = merger;
		...
		List<Integer> parent = rootChunck.worker2.parent;
		...
		for( Chunck<T> chunck : tree )
		{
			if ( chunck.children.size() == 0  )
			{
				RleImg partialRleImg = new RleImgView( runImgFull , chunck );
	...
	class RleLabeler extends RleWorker {
		...
		RleLabeler( Chunck<T> chunck, RleImg runImgFull, RandomAccessibleInterval<T> rai)
		{
			this.chunck = chunck;
			this.runImgFull = runImgFull;
		...
		public Chunck<T> call() throws Exception {
			...
			RleImg partialRunImg = new RleImgView( runImgFull, chunckedRai, threshold );
			
			RleCCL<T> labeler = new RleCCL<T>( partialRunImg );
			labeler.run();
			//runImg = partialRunImg;
	...
	class RleMerger extends RleWorker
	{
		...
		public RleMerger(Chunck<T> chunck, RleImg runImgFull)
		{
			this.chunck = chunck;
		...
		public Chunck<T> call() {
			...
			RleImg rleImgChunck = new RleImgView( runImgFull , chunck );
			List<Integer> labelEq0 = chunck0.worker2.getLabelEquivalence();
			List<Integer> labelEq1 = chunck1.worker2.getLabelEquivalence();
	        ...
	        ////////////////////////////////////////////////////////
			RleImg rleImg1 = new RleImgView( runImgFull , chunck.children.get(1) );
			...
			long[] rleDimChunck = rleImgChunck.getRleDim();
			...
			long[] rleDimFull = runImgFull.getRleDim();
			neighLinearOffset = Pixel.getIdxOffsetToCenterPix(neighDeltaPos, rleDimFull );
	        ...
	        long[] hDim = new long[ rleDim1.length];
	        long nPos = 1;
	        for ( int d=0 ; d<rleDim1.length ; d++ ) {
	        	if ( d == mergeDim-1 )
	        		hDim[d]=1;
	        	else
	        		hDim[d] = rleDim1[d];
	        ...
	        long[] posChunck = new long[rleDim1.length];
	        for( int ih=0 ; ih<nPos ; ih++) {
	        	// convert ih to a position in rleImg1, no translation needed since hyperslice and rleImg1 have the same origin
				...
				posChunck = pos1;
				posChunck[mergeDim-1] += rleImgChunck.getDim()[mergeDim];
				
				// get the neighbor lines of current line in rleImg1
				final List<List<PixelRun>> neighborLinesChunck = getNeighborLines(posChunck, rleDimChunck, rleImgChunck);
				...
				updateLabelEquivalence( currentLine1 , neighborLinesChunck);
	...
	public static void main(final String... args) throws IOException
	{
		...
		Img<FloatType> img =ImageJFunctions.wrap( IJ.openImage("C:/Users/Ben/workspace/testImages/blobs32.tif"));//noise5000_std50_blur10.tif") );
		...
		{
			RleCCL_multithreaded<FloatType> labelParallel1 = new RleCCL_multithreaded<FloatType>(img, threshold, nThread);
			labelMap1 = labelParallel1.getLabelMap();
			long dt = labelParallel1.getProcessTime();
			if( i>=warmup)
				dt1 += dt;
			min_dt1 = min_dt1 > dt ? dt : min_dt1;
			
			RleCCL_multithreaded2<FloatType> labelParallel2 = new RleCCL_multithreaded2<FloatType>(img, threshold, nThread);
			labelMap2 = labelParallel2.getLabelMap();
			dt = labelParallel2.getProcessTime();
			if( i>=warmup)
				dt2 += dt;
